...
void drawButton(SDL_Renderer *renderer, Button *button) {
    ...
    SDL_Color textColor = {0, 0, 0, 0}; // Couleur du texte (noir ici)
...
Mix_FreeChunk(fondmenu); 
Mix_FreeChunk(fondplay);
Mix_FreeChunk(debutjeu);
Mix_FreeChunk(tapemot);
Mix_FreeChunk(succesmot);
Mix_FreeChunk(echecmot);
Mix_FreeChunk(pertevie);
